...
@section('content')
    <div class="container">
        <form class="bg-white shadow border p-4" action="{{ route('users.destroy', ['user' => $user]) }}" method="POST">
            @csrf
            @method('DELETE')
            <h2>¿Está Seguro de Eliminar el Usuario?</h2>
            <ul>
                <li><h3><strong>Nombre:</strong> {{ $user->name }}</h3></li>
                <li><h3><strong>Correo:</strong> {{ $user->email }}</h3></li>
            </ul>
